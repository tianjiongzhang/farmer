...
window.onload = function()   
 {  
  labels = document.getElementById('formRadio').getElementsByTagName('label');  
  radios = document.getElementById('formRadio').getElementsByTagName('input');  
...
//长度校验
 function checkLength(obj, maxLength) {
 	...
 	if (leftLength <= 0) {
 		...
 		$(obj).next(".errors").html("<font color=\"green\">还可录入" + leftLength + "个字</font>");
 	} else {
 		$(obj).next(".errors").html("<font color=\"green\">还可录入" + leftLength + "个字</font>");
 	}
 }
 ...
 function showError(obj, msg) {
 	obj.next(".errors").html("<font color=\"#EA5200\" style=\"font-size: 14px;\"><b>" + msg + "</b></font>");
 }
 function isNull(obj)
 {
 ...
 {
 	return isNull(str) || (typeof(str) == 'string' && (str.replace(/(^\s+)|(\s+$)/g, '') == ''));
 }
